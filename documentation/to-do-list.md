...
- [ ]  **(Lead: Developer)** Review and finalize all documentation files (`Blueprint.docx`, `Tech-Stack.docx`, `Info-Checklist.docx`, `Cost-Estimate.docx`, `Timeline.docx`).
- [ ]  **(Lead: Developer)** Finalize Project Blueprint document with all agreed-upon features.
- [ ]  **(Lead: Developer)** Send initial inquiry email to SIUE IT department regarding SSO, student data access, and hosting.
...
- [ ]  **(Lead: Broadcast Engineers)** Begin collecting branding assets (SIUE/Mass Comm logos). *(Can be done in parallel with other phases)*
...
- [ ]  **(Lead: Developer)** Hold kickoff meeting with all stakeholders to approve the final blueprint and timeline.
...
- [ ]  **(Lead: Developer)** Set up environment variables and secrets for backend and frontend (local and production).
- [ ]  **(Lead: Developer)** Configure the `docker-compose.yml` file for a local Snipe-IT instance.
- [ ]  **(Lead: Developer)** Generate and configure the `APP_KEY` for the local Snipe-IT server.
